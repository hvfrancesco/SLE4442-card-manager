...
from PySide2.QtWidgets import QTableWidgetItem, QApplication, QMainWindow
...
class MyUi(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.PIN_PROT = [self.ui.bit_8, self.ui.bit_7, self.ui.bit_6, self.ui.bit_5, self.ui.bit_4, self.ui.bit_3,
                         self.ui.bit_2, self.ui.bit_1,
                         self.ui.bit_16, self.ui.bit_15, self.ui.bit_14, self.ui.bit_13, self.ui.bit_12, self.ui.bit_11,
                         self.ui.bit_10, self.ui.bit_9,
                         self.ui.bit_24, self.ui.bit_23, self.ui.bit_22, self.ui.bit_21, self.ui.bit_20, self.ui.bit_19,
                         self.ui.bit_18, self.ui.bit_17,
                         self.ui.bit_32, self.ui.bit_31, self.ui.bit_30, self.ui.bit_29, self.ui.bit_28, self.ui.bit_27,
                         self.ui.bit_26, self.ui.bit_25]

        # segnali
        QObject.connect(self.ui.leggi, SIGNAL("clicked()"), self.leggi_tutto)
        QObject.connect(self.ui.scrivi, SIGNAL("clicked()"), self.scrivi_tutto)
        QObject.connect(self.ui.connetti, SIGNAL("clicked()"), self.connetti)
        QObject.connect(self.ui.disconnetti, SIGNAL("clicked()"), self.disconnetti)
        QObject.connect(self.ui.sblocca, SIGNAL("clicked()"), self.sblocca)
        QObject.connect(self.ui.change_pin, SIGNAL("clicked()"), self.cambia_pin)
        QObject.connect(self.ui.proteggi, SIGNAL("clicked()"), self.proteggi_byte)
    ...
    def connetti(self):
        try:
            ...
            try:
                ...
                self.reader = readers[len(readers) - 1]
    ...
    def leggi_tutto(self):
        try:
            ...
            if response[-2] == 144:
                ...
                for i in range(255):
                    self.ui.dati.setItem(i / 8, i % 8, QTableWidgetItem(chr(risultato[i])))
            ...
            if response[-2] == 144:
                ...
                for i in range(32):
                    if risultato[i] == "1":
                        ...
                        byte = (i / 8) * 8 + 8 - (i % 8) - 1
                        self.ui.dati.item(byte / 8, byte % 8).setBackground(QColor(240, 240, 240))
                    elif risultato[i] == "0":
                        self.PIN_PROT[i].setChecked(True)
                        byte = (i / 8) * 8 + 8 - (i % 8) - 1
                        self.ui.dati.item(byte / 8, byte % 8).setBackground(QColor(150, 100, 100))
                    else:
    ...
    def sblocca(self):
        try:
            ...
            if len(self.PIN) != 3:
                ...
                hresult, response = SCardTransmit(self.hcard, self.dwActiveProtocol,
                                                  SBLOCCA_SCRITTURA + smartcard.util.toASCIIBytes(self.PIN))
                if hresult != SCARD_S_SUCCESS:
                    raise Exception('Failed to transmit: ' + SCardGetErrorMessage(hresult))
                if response[-1] == 7:
    ...
    def cambia_pin(self):
        try:
            ...
            if len(self.PIN) != 3:
                ...
                hresult, response = SCardTransmit(self.hcard, self.dwActiveProtocol,
                                                  CAMBIA_PIN + smartcard.util.toASCIIBytes(self.PIN))
                if hresult != SCARD_S_SUCCESS:
                    raise Exception('Failed to transmit: ' + SCardGetErrorMessage(hresult))
                if response[-2] == 144:
